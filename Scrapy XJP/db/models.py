...
class spider_mun(spider):
    ...
    def get_download_link(self,url, basePath):
        try:
            ...
            pdfNameList = []
            pdfLinkList = []
            # in case there's no pdf file, other kinds of files - ZIP or Video
            if not item:
                # in case that the files are neither PDF nor video, like ZIP
                url1 = '/'.join(url.split('/')[:-3])+'/topics/files/download/'+url.split('/')[-2]+'/DirectFileTopicDownload'

                self.updateURL(url1)
                if self.requests_web(): # test to check if this url1 exists
    ...
    def download_file(self,folderName,fileName, fileLink):
        try:
            myfile = requests.get(fileLink,cookies=self._cookies) # MUST have cookies here
